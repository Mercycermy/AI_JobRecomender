...
def categorize_job(title):
    ...
    if any(x in t for x in ['developer', 'engineer', 'architect', 'tech', 'software', 'app']):
        ...
        return 'creative'
    elif any(x in t for x in ['nurse', 'doctor', 'medical']):
        return 'medical'
    elif any(x in t for x in ['tutor', 'teacher']):
        return 'education'
    else:
        return 'general'
        ...
        "job_evidence": [
            {
                ...
                "dataset_frequency_note": "Top 100 Dataset Job",
                "source_url": None,
...
def main():
    try:
        ...
        top_100 = df.head(100)['job_title'].tolist()
        ...
        new_questions = []
        for i, title in enumerate(top_100, 1):
        ...
        print(f"Generated and appended {len(new_questions)} questions for 100 job titles.")
